  ...
  <v-table class="ma-5 mr-16" style="overflow: scroll; width : 100%;">
    <thead>
      <tr style="background-color: #1b3a4c; color: white !important;">
        ...
        <template v-for="(elt,index) in header" :key="index">
          <th class="text-center" style="color: white !important;">
            <slot
              :name="`header(${elt.value?elt.value:elt.key})`"
              >{{elt.text}}</slot
            >
          </th>
        </template>
    ...
    <tbody style="background-color: white!important">
      <template v-if="items.length===0">
        <tr>
          <td
            :colspan="with_selected?''+(header.length+1):''+(header.length)"
      ...
      <template v-else>
        ...
        <template v-else>
          <template v-for="(element,index) in items_display" :key="element.id">
            <tr>
              <td v-if="with_selected">
                <v-btn
                  variant="plain"
                  @click="cliquer_sur_un_element(element)"
                  :icon="est_dans_le_tableau(element)?'mdi mdi-checkbox-marked':'mdi mdi-checkbox-blank-outline'"
                ></v-btn>
              </td>

              <template v-for="(elt,index_elt) in header" :key="index_elt">
                <td
                  v-if="elt.is_to_expanded"
                  style="text-align:center; vertical-align: middle;"
                >
                  <v-btn
                    variant="plain"
                    @click="expanded_un_element(element)"
                    :icon="est_dans_expanded(element)?'mdi mdi-chevron-up':'mdi mdi-chevron-down'"
                  ></v-btn>
                </td>
                <td v-else>
                  <slot
                    :name="`item(${elt.value?elt.value:elt.key})`"
                    :item="getValueFromPropertyString(element,elt.value?elt.value:elt.key)"
                    :elt_item="element"
                    :index="index"
                    >{{getValueFromPropertyString(element,elt.value?elt.value:elt.key)}}</slot
                  >
                </td>
              </template>
            </tr>
            <tr v-if="est_dans_expanded(element)" class="py-1">
              <td
                ...
                class="px-5 pb-2"
              >
                <slot name="expanded" :item="element"> </slot>
              </td>
            </tr>
          </template>
        </template>
...
defineEmits(['update:title','update:items','update:selected','update:expanded','update:filter','update:header'])
