<template>
    ...
    <v-card class="mx-2 my-2" v-if="display_table">
      ...
      <va-data-table
        ...
        style="height: auto"
        no-data-html="Aucune élément trouvé."
        no-data-filtered-html="Aucune élément correspondant."
      >
        ...
        >
          <slot :name="`cell(${element.key})`" :value="value"> </slot>
        </template>
        <template #bodyAppend>
          <tr>
            <td colspan="8">
              <div class="table-example--pagination my-1">
                <va-pagination v-model="currentPage" input 
                :pages=pages />
              </div>
            </td>
          </tr>
  ...
  // console.log("ici c'est la valeur de l'objezt à prendre  la taille",items_object.value.items.length)
  
  const pages = computed(() => {
    return perPage.value && perPage.value !== 0
      ? Math.ceil(items_object.value.items.length / perPage.value)
      : items_object.value.items.length;
  });
  // console.log("ici c'est la valeur page",pages)
  ...
  async function valid_add_function(v) {
    ...
    if ((await v.form.validate()).valid) {
      ...
      else await default_add_function(v.item, items_object);
