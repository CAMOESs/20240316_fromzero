<template>
  <v-dialog v-model="dialog" persistent :max-width="width_dim">
    <template v-slot:activator="{ props }">
      <v-btn
        ...
        style="color: white"
        color="#1b3a4c"
        @click="click_to_acess_update"
      >
        <v-icon> {{ icon_of_button }} </v-icon
        ><span class="d-none d-xs-none d-sm-block">{{ name_of_button }}</span>
      </v-btn>
    </template>
    <v-form @submit.prevent="submit" ref="form" class="overflow-y-auto">
      <v-card class="px-6" style="width: 300; text-align: center">
        <slot name="button_text" :item="item"> </slot>
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn
            color="secondary"
...
const hidden_btn = computed(() => {
  if (!props.hidden_close_btn) return true;
